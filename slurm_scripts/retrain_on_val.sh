...
echo "Starting retrain_on_val.py on ${config}"
echo "Additional python args: ${pyth_args}"

# Read experiment directory name from the config json
experiment_name=$(python -c "from retrain_on_val import build_argparser; \
                             print(build_argparser().parse_args().experiment_name)" \
                  --config=$json_config \
                  ${pyth_args})
exp_group=$(python -c "from retrain_on_val import build_argparser; \
                       print(build_argparser().parse_args().exp_group)" \
            --config=$json_config \
            ${pyth_args})
full_exp_name=$(python -c "from retrain_on_val import build_argparser; \
...
COMMAND="python retrain_on_val.py --gpus='0' --config=$json_config --output_root_dir=${TMP} ${pyth_args}"
eval $COMMAND
