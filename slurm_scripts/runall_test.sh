...
echo "Starting run_all_test.py on ${config}"
echo "Additional python args: ${pyth_args}"

# Read experiment directory name from the config json
experiment_name=$(python -c "from run_all_test import build_argparser; \
                             print(build_argparser()._parse_known_args()[0].experiment_name)" \
                  --config=$json_config \
                  ${pyth_args})
exp_group=$(python -c "from run_all_test import build_argparser; \
                       print(build_argparser()._parse_known_args()[0].exp_group)" \
            --config=$json_config \
            ${pyth_args})
full_exp_name=$(python -c "from run_all_test import build_argparser; \
                           from cdi.util.utils import construct_experiment_name; \
                           print(construct_experiment_name(build_argparser()._parse_known_args()[0]))" \
                --config=$json_config \
                ${pyth_args})
...
COMMAND="python run_all_test.py --gpus='0' --config=$json_config --output_root_dir=${TMP} ${pyth_args}"
eval $COMMAND
